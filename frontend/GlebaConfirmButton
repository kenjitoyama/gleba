...
class MainWindow(gtk.Window):
    ...
    def history_callback(self, button):
        print "History callback run"
        if self.showWeight:
            if self.currentState == AWAITING_CONFIRMATION:
                self.change_state()
		...
		entry += ", Variety: " + self.varieties[self.currentVariety][0]
                ...
                self.historyList.set_model(self.historyStore)
                self.show_all()
        print "History callback finished"
    ...
    def count_up(self, maximum):
        while (self.keepRunning==True):
            ...
            elif self.currentState == AWAITING_BOX and self.currentWeight > 0.5:
                gobject.idle_add(self.change_state)
                gobject.idle_add(self.set_status_feedback)
            ...
            if self.showWeight==True:
                ...
                # If box weight is within acceptable range
                    ...
                    self.currentState = AWAITING_CONFIRMATION
                    self.currentWeight = self.ts.getWeight()
                    #gobject.idle_add(self.set_status_feedback)
                    #time.sleep(2)
    ...
    def change_state(self):
        ...
        elif self.currentState==AWAITING_CONFIRMATION:
            print "Awaiting confirmation entered"
    ...
    def start_stop(self, sound):
        if sound=="success":
            self.player.set_property("uri", "file:///home/gleba/Desktop/success.ogg")
        elif sound=="green":
            self.player.set_property("uri", "file:///home/gleba/Desktop/green.ogg")
        elif sound=="button":
            self.player.set_property("uri", "file:///home/gleba/Desktop/button.ogg")
